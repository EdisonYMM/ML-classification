...
'''class DataManager():
    ...
    def read(self,name,path):
        with open(path,newline = '') as csvfile:
            ...
            if name == 'X_train':
                self.mean = np.mean(rows,axis = 0).reshape(1,-1) ##对各列求平均值并转换成一行
                self.std = np.std(rows,axis = 0).reshape(1,-1)
                self.theta = np.ones((rows.shape[1]+1,1),dtype = float)
                for i in range(rows.shape[0]):
                    rows[i,:] = (rows[i,:]-self.mean)/self.std ##每一行进行normalization运算
    ...
    def find_theta(self):
        ...
        mean_1 = np.mean(class_1,axis = 0) ## 两个class分别求平均
        ...
        cov = (cov_0*class_0.shape[0]+cov_1*class_1.shape[1])/(class_0.shape[0]+class_1.shape[0])
        self.w = np.transpose(((mean_0-mean_1)).dot(inv(cov)))
        self.b = (-0.5)*(mean_0).dot(inv(cov)).dot(mean_0)+(0.5)*(mean_1).dot(inv(cov))+np.log(float(class_0.shape[0])/class_1.shape[0])
